...
def create_json(database_path, json_path):
    ...
    #! simulation_descriptions -> simulation_descriptions_4fix_vary
    simulation_descriptions = \
        load_json(Path(database_path).joinpath('simulation_descriptions_4fix_vary.json')) 
    for scenario in simulation_descriptions.values():
        ...
        for example_id, example in scenario.items():
            ...
            if 'src_diary_aux' in example:
                example['src_diary_aux'] = [
                complete_source_information(
                    source, example_id, setups, rir_root
                )
                for source in example['src_diary_aux']]
