...
def generate_audio(example,
    ...
    #! ----------------------------------------------------------------- #
    #! Changed by SGuan - gshanzheng@foxmail.com
    if 'src_diary_aux' in example:

        src_diary = example['src_diary_aux']
        ...
        # trim data.
        min_len      = min(audio_data.shape[1], audio_data_.shape[1])
        audio_data_  = audio_data_[:, :min_len]
        audio_data   = audio_data[:,  :min_len]

        audio_data_ = audio_data_ + audio_data
    #! ----------------------------------------------------------------- #

    #! ----------------------------------------------------------------- #
    #! Changed by SGuan - gshanzheng@foxmail.com
    #! load diffuse babble noise --------------------------------------- #
    root_path_diff = '-------/ANF-Generator-BabbleNoise/babble_noise_100_SV/'
    diffuse_audio = load_audio(root_path_diff+example['example_id']+'.wav')[int(node_id[-1]):int(node_id[-1])+1, :]

    std_signal       = std_sensor_noise * 10**(30/20)

    if 'snr_diff' in example:

        target_snr_db = np.array(example['snr_diff'])
        
        target_noise_std = std_signal / (10**(target_snr_db / 20))
        current_std      = np.std(diffuse_audio)
        scaling_factor   = target_noise_std / current_std
        # trim data
        min_len         = min(diffuse_audio.shape[1], audio_data_.shape[1])
        audio_data_     = audio_data_[:,   :min_len]
        diffuse_audio   = diffuse_audio[:, :min_len]

        audio_data_     = audio_data_ + scaling_factor * diffuse_audio
    else:
        pass
    #! ----------------------------------------------------------------- #
